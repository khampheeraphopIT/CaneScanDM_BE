...
def compute_all_features(image_path, province=None):
    ...
    features = [
        float(weather_features[0]),
        float(weather_features[1]),
        float(weather_features[2]),
        float(vegetation_indices['vari']),
        float(vegetation_indices['exg']),
        float(vegetation_indices['cive']),
        float(glcm_features['contrast']),
        float(glcm_features['homogeneity']),
        float(glcm_features['energy']),
        float(lbp_feature),
        float(edge_density)
    ]
    # Removed fit_transform because it corrupts single sample data
