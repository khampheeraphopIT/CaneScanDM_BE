...
def compute_all_features(image_path, province=None):
    ...
    return np.array(features, dtype=np.float32), weather_dict
...
def prepare_data(csv_path):
    ...
    logger.info("Finished computing numerical features and scaling")
    return image_paths, labels, numerical_data, scaler
...
def train_model(csv_path, model_dir='model', fine_tune=False):
    ...
    X, y, num_data, scaler = prepare_data(csv_path)
    ...
    scaler_path = os.path.join(model_dir, f"scaler_{timestamp}.json")
    with open(scaler_path, 'w') as f:
        json.dump(scaler_params, f)
    logger.info(f"Scaler parameters saved as: {scaler_path}")

    return model, model_path

if __name__ == "__main__":
    os.makedirs("model", exist_ok=True)
    # Use path relative to root when running as a module or from root
    dataset_path = "dataset_updated.csv"
    if not os.path.exists(dataset_path):
        # Fallback for running from src/model
        dataset_path = "../../dataset_updated.csv"
    model = train_model(dataset_path, fine_tune=False)